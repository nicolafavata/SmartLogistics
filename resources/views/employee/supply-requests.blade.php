...
@section('content')
    <div class="carousel-inner request-supply_img">
        ...
        <div class="container admin_home">
            <div class="row">
                <h1 class="fucsia font-weight-bold shadow uppercase">Le richieste di aggregazione trasmesse</h1>
            </div>
                    ...
                    @forelse($company as $found)
                            ...
                            <div class="row">
                                <div class="col-md-6 text-right">
                                    <form onsubmit="showloader()" method="POST" action="{{ route('cancel-request', $found->company_received) }}">
                                        {{ csrf_field() }}
                                        <button type="submit" class="btn btn-primary2">Annulla richiesta</button>
                                    </form>
                                </div>
                                <div class="col-md-6 text-left">
                                    <form onsubmit="showloader()" method="POST" action="{{ route('retransmit-request', $found->company_received) }}">
                                        {{ csrf_field() }}
                                        <button type="submit" class="btn btn-primary2">Ritrasmetti email</button>
                                    </form>
                                </div>
                            </div>
                            <hr>
